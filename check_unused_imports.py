...
def is_likely_used(name: str, content: str, checker: SmartImportChecker) -> bool:
    ...
    if name in platform_imports and any(x in content for x in ['if', 'try', 'platform', 'darwin', 'linux', 'win']):
        return True
    
    # Type hints için kullanılan importlar
    if any(x in content for x in ['Dict', 'List', 'Set', 'Tuple', 'Optional', 'Union']) and name in ['typing', 'Dict', 'List', 'Set', 'Tuple', 'Optional', 'Union']:
        return True
    
    # __all__ export listesinde geçiyor mu
    if '__all__' in content and name in content[content.find('__all__'):content.find('__all__') + 500]:
        return True
        ...
        for module, aliases in checker.imports.items():
            for alias in aliases:
                if not is_likely_used(alias, content, checker):
                    unused.append(f"import {module}" + (f" as {alias}" if alias != module.split('.')[-1] else ""))
        
        # From imports kontrol et
        for module, imports in checker.from_imports.items():
            for name, alias in imports:
                if not is_likely_used(alias, content, checker):
                    unused.append(f"from {module} import {name}" + (f" as {alias}" if alias != name else ""))

...
Bu dosya, PeakActivity projesinin veritabanı optimizasyonu ve query performance
  ...
  "fieldOverrides": [
    {
      ...
      "indexes": [
        {
          "queryScope": "COLLECTION",
          "fields": [{ "fieldPath": "data.window_title", "order": "ASCENDING" }]
        }
      ]
    }
  ]
...
export class QueryOptimizer {
   ...
   */
  async getDailyAggregates(userId: string, dateKey: string) {
...
export class QueryAnalytics {
  ...
  static getQueryPerformanceSummary() {
    const summary = Array.from(this.queryMetrics.entries()).map(
      ([name, metrics]) => ({
        queryName: name,
        ...metrics,
...
export const optimizedUserService = {
  async getUserActivity(userId: string, timeRange: any) {
    return QueryAnalytics.executeWithMetrics('getUserActivity', () =>
...
export class DataRetentionManager {
  ...
  private static async deleteOldEventsForUser(
    userId: string,
    cutoffDate: Date
  ) {
    const db = getFirestore();
...
Bu database optimization patterns ile Firestore query performance'ınızı önemli
ölçüde artırabilir ve maliyetleri kontrol altında tutabilirsiniz.

...
Bu dosya, PeakActivity projesi için test stratejileri ve kalite güvence
...
describe('Firebase Functions Integration', () => {
  ...
  it('Firestore güvenlik kuralları doğru çalışmalı', async () => {
    ...
    const userDoc = authenticatedContext
      .firestore()
      .collection('users')
      .doc('user123');
    ...
    const unauthorizedDoc = unauthenticatedContext
      .firestore()
      .collection('users')
      .doc('user123');
...
test.describe('Kullanıcı İş Akışı', () => {
  ...
  test('aktivite verilerini görüntüleyebilmeli', async ({ page }) => {
    ...
    await expect(page.locator('[data-testid="filtered-events"]')).toBeVisible();
  });
  ...
  test('AI önerilerini alabilmeli', async ({ page }) => {
    ...
    await expect(
      page.locator('[data-testid="insights-loading"]')
    ).toBeVisible();

    // Sonuçların yüklenmesini bekle
    await expect(page.locator('[data-testid="insights-results"]')).toBeVisible({
      timeout: 10000,
    });
...
export default defineConfig({
  test: {
    coverage: {
      ...
      exclude: ['tests/**', '**/*.test.ts', '**/*.spec.ts', '**/types/**'],
    },
  },
});
```
...
sonar.projectKey=peakactivity sonar.organization=peakactivity-org
sonar.sources=src sonar.tests=tests sonar.exclusions=**/*.test.ts,**/*.spec.ts

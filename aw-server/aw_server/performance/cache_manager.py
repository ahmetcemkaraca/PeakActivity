...
class CacheManager:
    ...
    def __init__(self, backend: Optional[CacheBackend] = None):
        ...
        self.query_cache = TTLCache(maxsize=100, ttl=300) if CACHETOOLS_AVAILABLE else {}
        self.event_cache = LRUCache(maxsize=1000) if CACHETOOLS_AVAILABLE else {}
        self.bucket_cache = TTLCache(maxsize=50, ttl=3600) if CACHETOOLS_AVAILABLE else {}
        self.compute_cache = TTLCache(maxsize=200, ttl=600) if CACHETOOLS_AVAILABLE else {}
    ...
    def cached_computation(self, ttl: int = 600, invalidate_on: Optional[List[str]] = None):
        ...
        def decorator(func: F) -> F:
            @functools.wraps(func)
            def wrapper(*args, **kwargs):
                cache_key = self._generate_cache_key(f"compute_{func.__name__}", args, kwargs)
                ...
                logger.debug(
                    ...
                    func.__name__, execution_time, cache_key[:16]
                )
                return result
    ...
    def invalidate_pattern(self, pattern: str) -> int:
        ...
        logger.info("Invalidated %d cache entries matching pattern: %s", invalidated, pattern)
        return invalidated

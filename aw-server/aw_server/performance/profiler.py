...
class PerformanceProfiler:
    ...
    def get_most_called(self, count: int = 10) -> List[PerformanceMetrics]:
        ...
        with self._lock:
            sorted_metrics = sorted(
                self._metrics.values(), key=lambda m: m.call_count, reverse=True
            )
            return sorted_metrics[:count]
    ...
    def generate_report(self, include_system: bool = True) -> str:
        ...
        if include_system and PSUTIL_AVAILABLE:
            ...
            if system_metrics:
                ...
                report.add_bullet(
                    f"Memory Usage: {latest.memory_percent:.1f}% ({latest.memory_rss_mb:.1f} MB)"
                )
                report.add_bullet(
                    f"Disk I/O: {latest.disk_io_read_mb:.1f} MB read, {latest.disk_io_write_mb:.1f} MB write"
                )

                if len(system_metrics) > 1:
                    avg_cpu = sum(m.cpu_percent for m in system_metrics) / len(
                        system_metrics
                    )
    ...
    def _update_metrics(
        self, name: str, execution_time: float, error: bool = False
    ) -> None:
        ...
        with self._lock:
            ...
            self._history[name].append(
                {"time": time.time(), "execution_time": execution_time, "error": error}
            )

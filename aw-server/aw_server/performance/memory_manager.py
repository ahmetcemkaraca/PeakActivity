...
class MemoryMonitor:
    ...
    def __init__(self, warning_threshold: float = 80.0, critical_threshold: float = 90.0):
        ...
        self._callbacks: Dict[str, List[Callable[[MemoryStats], None]]] = defaultdict(list)
    ...
    def register_callback(self, level: str, callback: Callable[[MemoryStats], None]) -> None:
        ...
        with self._lock:
            ...
            for category in self._tracked_objects:
                ...
                if len(counts) >= 3:
                    ...
                    if (all(recent_counts[i] <= recent_counts[i+1] for i in range(len(recent_counts)-1)) and
                        recent_counts[-1] >= threshold):
...
class MemoryOptimizer:
    ...
    def optimize_gc() -> Dict[str, Any]:
        ...
        logger.info("GC optimization: freed %d objects, %d -> %d total objects", 
                   result['objects_freed'], before_objects, after_objects)
        
        return result
    
    @staticmethod
    def memory_efficient_chunking(data: List[T], chunk_size: int = 1000) -> Generator[List[T], None, None]:
...
def setup_memory_optimization() -> None:
    ...
    monitor.register_callback('warning', lambda stats: 
        logger.warning("Memory usage high: %.1f%% (%.1f MB)", stats.percent, stats.rss_mb))
    monitor.register_callback('critical', lambda stats: 
        logger.critical("Memory usage critical: %.1f%% (%.1f MB)", stats.percent, stats.rss_mb))
    monitor.start_monitoring()
    
    # Start resource management
    resource_manager = get_resource_manager()
    resource_manager.register_periodic_task(lambda: gc.collect(), 60.0)  # GC every minute
    ...
    def wrapper(*args, **kwargs):
        ...
        try:
            ...
            logger.info(
                ...
                func.__name__, memory_delta, time_delta
            )

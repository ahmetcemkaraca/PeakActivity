...
from aw_server.data_anonymization.anonymizer import (
    Anonymizer,
)  # Anonymizer sınıfını içe aktar
...
class FirestoreEventDB(EventDB):
    ...
    def get(
        ...
        """
        query = self.collection_ref.order_by("timestamp")
        if start:
    ...
    def get_total_duration(
        ...
        aggregate_query = aggregation.AggregationQuery(query)
        aggregate_query.sum("duration", alias="total_duration")
    ...
    def get_average_duration(
        ...
        aggregate_query = aggregation.AggregationQuery(query)
        aggregate_query.avg("duration", alias="average_duration")
...
class FirestoreStorage(Storage):
    ...
    def __init__(
        self, user_id: str, testing: bool = False, anonymize_data: bool = False
    ):
        ...
        self.user_id = user_id
        self.buckets_collection_ref = (
            firestore_db.collection("users").document(user_id).collection("buckets")
        )
        self.anonymize_data = anonymize_data  # anonymize_data eklendi
    ...
    def create_bucket(
        ...
        bucket_data = {
            ...
            "last_updated": datetime.now(),  # Yeni eklenen alan
        }
    ...
    def update_bucket(
        ...
        if data is not None:
            updates["data"] = data
        updates["last_updated"] = datetime.now()  # Güncelleme zamanı

...
class Anonymizer:
    def __init__(self, config=None):
        self.config = (
            ...
            else {"title": {"method": "hash"}, "app": {"method": "hash"}}
        )
        ...
        for field, options in self.config.items():
            if field in anonymized_data:
                ...
                if method == "hash":
                    ...
                    anonymized_data[field] = self._truncate_data(
                        anonymized_data[field], length
                    )
                elif method == "redact":
                    pattern = options.get(
                        "pattern", "(?<=[a-zA-Z0-9]{3})[a-zA-Z0-9](?=[a-zA-Z0-9]{3})"
                    )  # Varsayılan olarak ortadaki karakterleri gizler
                    replace_with = options.get("replace_with", "*")
                    anonymized_data[field] = self._redact_data(
                        anonymized_data[field], pattern, replace_with
                    )
                elif method == "randomize":
                    data_type = options.get("type", "numeric")
                    anonymized_data[field] = self._randomize_data(
                        anonymized_data[field], data_type
                    )

...
export const useAIInsightsStore = defineStore('aiInsights', () => {
  ...
  const processingStatus = ref<Map<string, {
    inProgress: boolean;
    progress: number;
    lastUpdate: Date;
  }>>(new Map());
  ...
  const unreadInsights = computed(() => 
    insights.value.filter(insight => !insight.read)
  );
  ...
  // Actions
  const addInsight = (insight: Omit<typeof insights.value[0], 'id' | 'timestamp' | 'read'>): void => {
    const newInsight = {
      ...insight,
  ...
  const clearOldInsights = (daysOld: number = 30): void => {
    ...
    insights.value = insights.value.filter(insight => 
...
export const useAIAutomationStore = defineStore('aiAutomation', () => {
  // State
  const automationRules = ref<Array<{
    id: string;
    name: string;
    description: string;
    enabled: boolean;
  ...
  const activeRules = computed(() => 
    automationRules.value.filter(rule => rule.enabled)
  );
  ...
  // Actions
  const addAutomationRule = (rule: Omit<typeof automationRules.value[0], 'id' | 'triggerCount'>): string => {
  ...
  const updateAutomationRule = (ruleId: string, updates: Partial<typeof automationRules.value[0]>): void => {
...
export const useAIServiceStore = defineStore('aiService', () => {
  ...
  const updateModelStatus = (modelId: string, status: typeof availableModels.value[0]['status']): void => {
    const model = availableModels.value.find(m => m.id === modelId);
    if (model) {
      model.status = status;
    }

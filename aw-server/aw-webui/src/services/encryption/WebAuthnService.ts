import { WebAuthnRegistrationRequest, WebAuthnRegistrationResponse, WebAuthnAuthenticationRequest, WebAuthnAuthenticationResponse, WebAuthnCredential } from "../../types/webauthn.d";
...
export class WebAuthnService {
     ...
     */
    public async register(request: WebAuthnRegistrationRequest): Promise<WebAuthnRegistrationResponse | null> {
        ...
        try {
            ...
            const response: WebAuthnRegistrationResponse = {
                attestationObject: this.arrayBufferToBase64Url(credential.response.attestationObject as ArrayBuffer),
                clientDataJSON: this.arrayBufferToBase64Url(credential.response.clientDataJSON as ArrayBuffer),
            };
     ...
     */
    public async authenticate(request: WebAuthnAuthenticationRequest): Promise<WebAuthnAuthenticationResponse | null> {
        ...
        try {
            ...
            const response: WebAuthnAuthenticationResponse = {
                authenticatorData: this.arrayBufferToBase64Url(assertion.response.authenticatorData as ArrayBuffer),
                clientDataJSON: this.arrayBufferToBase64Url(assertion.response.clientDataJSON as ArrayBuffer),
                signature: this.arrayBufferToBase64Url(assertion.response.signature as ArrayBuffer),
                userHandle: assertion.response.userHandle ? this.arrayBufferToBase64Url(assertion.response.userHandle as ArrayBuffer) : "",
            };

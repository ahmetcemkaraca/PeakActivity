...
class SafeFormatter:
    ...
    def format_safe(template: str, **kwargs: Any) -> str:
        ...
        try:
            ...
            logger.warning("String formatting failed for template '%s': %s", template, e)
    ...
    def format_safe_partial(template: str, **kwargs: Any) -> str:
        ...
        try:
            ...
            return safe_template.safe_substitute(**kwargs).replace('$', '{').replace('{', '}')
        except Exception as e:
            logger.warning("Partial formatting failed for template '%s': %s", template, e)
            return template

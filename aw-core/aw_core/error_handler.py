...
from .exceptions import (
    ...
    AWQueryException,
    AWConfigException,
    AWAuthenticationException,
    AWEncryptionException,
    AWFirebaseException,
    AWPlatformException,
    AWDatabaseException,
    AWPeeweeException,
    ...
    AWQProcessException,
    AWQTimerException,
    AWQSystemTrayException,
    AWConnectionException,
    AWTimeoutException,
    AWDNSException,
    AWSSLException,
    AWDisplayException,
...
class ErrorHandler:
    ...
    def register_recovery_strategy(
        cls, exception_type: Type[Exception], strategy: Callable
    ):
    ...
    def handle(cls, exception: Exception, context: Optional[str] = None) -> Any:
        ...
        # Default handling
        logger.error(
            f"Unhandled exception in {context or 'unknown context'}: {exception}"
        )
    ...
    def decorator(func: Callable):
        ...
        def wrapper(*args, **kwargs):
            ...
            if recovery_value is not None:
                logger.warning(
                    f"Returning recovery value for {func.__name__}: {recovery_value}"
                )
    ...
    def decorator(main_func: Callable):
        ...
        def wrapper(*args, **kwargs):
            try:
                ...
                logger.warning(
                    f"Main function {main_func.__name__} failed: {e}, using fallback"
                )
                try:
...
def handle_storage_error(
    exception: AWStorageException, context: Optional[str] = None
) -> None:
    """Storage error handler"""
    logger.error(
        f"Storage error{' in ' + context if context else ''}: {exception.message}"
    )
...
def handle_network_error(
    exception: AWNetworkException, context: Optional[str] = None
) -> None:
    """Network error handler"""
    logger.warning(
        f"Network error{' in ' + context if context else ''}: {exception.message}"
    )
    if exception.details:
        logger.debug(f"Network error details: {exception.details}")


def handle_window_error(
    exception: AWWindowException, context: Optional[str] = None
) -> Optional[Dict[str, str]]:
    """Window error handler - unknown window bilgisi döndür"""
    logger.warning(
        f"Window error{' in ' + context if context else ''}: {exception.message}"
    )
    return {"app": "unknown", "title": "unknown"}


def handle_input_error(
    exception: AWInputException, context: Optional[str] = None
) -> Dict[str, int]:
    """Input error handler - sıfır input döndür"""
    logger.warning(
        f"Input error{' in ' + context if context else ''}: {exception.message}"
    )
...
def handle_database_error(
    exception: AWDatabaseException, context: Optional[str] = None
) -> None:
    """Database error handler"""
    logger.error(
        f"Database error{' in ' + context if context else ''}: {exception.message}"
    )
...
def handle_peewee_error(
    exception: AWPeeweeException, context: Optional[str] = None
) -> None:
    """Peewee ORM error handler"""
    logger.error(
        f"Peewee ORM error{' in ' + context if context else ''}: {exception.message}"
    )
    logger.error(f"Operation: {exception.operation}, Table: {exception.table}")
    if exception.details:
...
def handle_database_connection_error(
    exception: AWDatabaseConnectionException, context: Optional[str] = None
) -> None:
    """Database connection error handler"""
    logger.error(
        f"Database connection error{' in ' + context if context else ''}: {exception.message}"
    )
...
def handle_database_integrity_error(
    exception: AWDatabaseIntegrityException, context: Optional[str] = None
) -> None:
    """Database integrity error handler"""
    logger.error(
        f"Database integrity error{' in ' + context if context else ''}: {exception.message}"
    )
    logger.error(f"Operation: {exception.operation}, Table: {exception.table}")
...
def handle_qprocess_error(
    exception: AWQProcessException, context: Optional[str] = None
) -> None:
    """QProcess error handler"""
    logger.error(
        f"QProcess error{' in ' + context if context else ''}: {exception.message}"
    )
...
def handle_connection_error(
    exception: AWConnectionException, context: Optional[str] = None
) -> None:
    """Connection error handler"""
    logger.warning(
        f"Connection error{' in ' + context if context else ''}: {exception.message}"
    )
    logger.warning(
        f"Host: {exception.host}, Port: {exception.port}, Timeout: {exception.timeout}"
    )


def handle_timeout_error(
    exception: AWTimeoutException, context: Optional[str] = None
) -> None:
    """Timeout error handler"""
    logger.warning(
        f"Timeout error{' in ' + context if context else ''}: {exception.message}"
    )
    logger.warning(
        f"Host: {exception.host}, Port: {exception.port}, Timeout: {exception.timeout}"
    )
...
def handle_dns_error(exception: AWDNSException, context: Optional[str] = None) -> None:
    """DNS error handler"""
    logger.error(
        f"DNS resolution error{' in ' + context if context else ''}: {exception.message}"
    )
...
def handle_xlib_error(
    exception: AWXlibException, context: Optional[str] = None
) -> None:
    """Xlib error handler"""
    logger.error(
        f"Xlib error{' in ' + context if context else ''}: {exception.message}"
    )
    logger.error(f"Display: {exception.display_name}")
...
def handle_file_error(
    exception: AWFileException, context: Optional[str] = None
) -> None:
    """File operation error handler"""
    logger.error(
        f"File error{' in ' + context if context else ''}: {exception.message}"
    )
    logger.error(f"File: {exception.file_path}, Operation: {exception.operation}")


def handle_permission_error(
    exception: AWPermissionException, context: Optional[str] = None
) -> None:
    """Permission error handler"""
    logger.error(
        f"Permission error{' in ' + context if context else ''}: {exception.message}"
    )
    logger.error(f"File: {exception.file_path}, Operation: {exception.operation}")
    # Permission fix önerileri
    logger.info(
        "Consider checking file permissions or running with appropriate privileges"
    )


def handle_disk_space_error(
    exception: AWDiskSpaceException, context: Optional[str] = None
) -> None:
    """Disk space error handler"""
    logger.error(
        f"Disk space error{' in ' + context if context else ''}: {exception.message}"
    )
    logger.error(f"File: {exception.file_path}, Operation: {exception.operation}")
    # Disk space cleanup önerileri
    logger.warning(
        "Insufficient disk space - consider cleaning up old data or increasing storage"
    )


# Setup specific error handlers
def handle_dependency_error(
    exception: AWDependencyException, context: Optional[str] = None
) -> None:
    """Dependency error handler"""
    logger.error(
        f"Dependency error{' in ' + context if context else ''}: {exception.message}"
    )
    logger.error(
        f"Dependency: {exception.dependency_name}, Required version: {exception.required_version}"
    )
    # Dependency resolution önerileri
    logger.info(
        f"Consider installing {exception.dependency_name} version {exception.required_version} or higher"
    )
...
ErrorHandler.register_handler(
    AWDatabaseConnectionException, handle_database_connection_error
)
ErrorHandler.register_handler(
    AWDatabaseIntegrityException, handle_database_integrity_error
)

...
export class KeyRecoveryService {
  ...
  public async initiateKeyRecovery(
    userId: string,
    ...
    if (verificationData.securityAnswers) {
      ...
      for (const userAnswer of userAnswers) {
        ...
        if (
          !stored ||
          !(await this.securityQuestionService.verifyAnswer(userAnswer.answer, stored.hashedAnswer))
        ) {
  ...
  public async resetMasterKey(userId: string, newPassword?: string): Promise<boolean> {
    ...
    if (newPassword) {
      ...
      console.warn(
        'Yeni parola belirtilmedi. Master anahtar rastgele oluşturulmayacak, mevcut anahtar kullanılacak veya manuel olarak ayarlanması gerekecek.'
      );
      return false;
    }

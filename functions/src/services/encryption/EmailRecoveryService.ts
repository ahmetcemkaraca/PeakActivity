...
export class EmailRecoveryService {
    ...
    public async generateRecoveryToken(userId: string): Promise<EmailRecoveryToken> {
        ...
        const expiration = new Date(Date.now() + this.TOKEN_EXPIRATION_HOURS * 60 * 60 * 1000).toISOString();
     ...
     */
    public async verifyRecoveryToken(userId: string, token: string): Promise<EmailRecoveryToken | null> {

...
export class TwoFactorManager {
    ...
    public async initiateTOTPSetup(userId: string): Promise<{ secret: string; uri: string }> {
        ...
        console.log(`Kullanıcı ${userId} için TOTP kurulumu başlatıldı. Gizli anahtar: ${secret}, URI: ${uri}`);
        return { secret, uri };
    }
     ...
     */
    public async verifyAndEnableTOTP(userId: string, secret: string, token: string): Promise<boolean> {
     ...
     * @returns Doğrulama başarılı olursa true, aksi takdirde false.
     */
    public async verifyAndEnableSms(userId: string, phoneNumber: string, code: string): Promise<boolean> {

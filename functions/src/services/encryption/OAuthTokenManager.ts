...
export class OAuthTokenManager {
    ...
    public async encryptAndSaveTokens(
        ...
        if (!masterKey) {
            throw new Error("Ana anahtar bulunamadı. Lütfen önce bir ana anahtar oluşturun veya oturum açın.");
        }
        ...
        const encryptedPayload = await this.encryptionService.encrypt(payload, masterKey, CryptoUtils.bytesToBase64(iv));
        ...
        await this.firestore.collection("users").doc(userId).collection("externalCredentials").doc(serviceName).set(encryptedCredentials);

        console.log(`Kullanıcı ${userId} için ${serviceName} OAuth tokenları başarıyla şifrelendi ve kaydedildi.`);
     ...
     */
    public async getEncryptedTokens(userId: string, serviceName: string): Promise<ExternalCredential | null> {
        console.log(`Kullanıcı ${userId} için ${serviceName} şifrelenmiş tokenları alınıyor.`);
        const doc = await this.firestore.collection("users").doc(userId).collection("externalCredentials").doc(serviceName).get();
     ...
     */
    public async refreshAccessToken(userId: string, serviceName: string, currentRefreshToken: string): Promise<ExternalCredential> {
    ...
    public async deleteTokens(userId: string, serviceName: string): Promise<boolean> {
        ...
        await this.firestore.collection("users").doc(userId).collection("externalCredentials").doc(serviceName).delete();
        return true;
    }
} 
    ...
    public async migrateExternalCredentials(userId: string): Promise<boolean> {
        ...
        try {
            ...
            if (oldGoogleAuthData && oldGoogleAuthData.accessToken && oldGoogleAuthData.refreshToken) {
                console.log(`Kullanıcı ${userId} için eski Google Calendar tokenları bulundu, şifreleniyor.`);
                const encryptedTokens = await this.googleCalendarEncryptionService.encryptGoogleCalendarTokens(
                    userId,
                ...
                console.log(`Kullanıcı ${userId} için eski Google Calendar tokenları bulunamadı veya zaten yeni formatta.`);
            }
        } catch (error) {
        ...
        try {
            ...
            if (oldTrelloAuthData && oldTrelloAuthData.apiKey && oldTrelloAuthData.token) {
                ...
                console.log(`Kullanıcı ${userId} için eski Trello kimlik bilgileri bulunamadı veya zaten yeni formatta.`);
            }
        } catch (error) {
        ...
        try {
            ...
            if (oldJiraAuthData && oldJiraAuthData.host && oldJiraAuthData.username && oldJiraAuthData.password) {
                ...
                console.log(`Kullanıcı ${userId} için eski Jira kimlik bilgileri bulunamadı veya zaten yeni formatta.`);
            }
        } catch (error) {
        ...
        console.log(`Kullanıcı ${userId} için dış kimlik bilgileri geçişi tamamlandı. Başarılı: ${success}`);
        return success;
    }
} 
...
export class BackupCodesService {
    ...
    public async hashBackupCodes(codes: string[]): Promise<string[]> {
        const hashedCodes: Promise<string>[] = codes.map(code => bcrypt.hash(code, this.HASH_SALT_ROUNDS));
    ...
    public async verifyAndInvalidateCode(userId: string, code: string): Promise<boolean> {
        ...
        if (codeVerified) {
            ...
            console.log(`Kullanıcı ${userId} için yedek kod başarıyla doğrulandı ve geçersiz kılındı (yer tutucu).`);
            return true;
        } else {

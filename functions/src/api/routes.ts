  ...
  asyncHandler(async (req: Request, res: Response) => {
    ...
    if (!title || !start_date || !due_date || !status) {
      throw new InvalidArgumentError(
        'Gerekli alanlar eksik: başlık, başlangıç tarihi, bitiş tarihi, durum.'
      );
    }
...
router.post(
  ...
  asyncHandler(async (req: Request, res: Response) => {
    ...
    if (type !== 'report' && type !== 'dashboard') {
      throw new InvalidArgumentError(
        'Geçersiz rapor türü. Sadece "report" veya "dashboard" olabilir.'
      );
    }
    const newReport = await reportManagementService.createReport(userId, {
      name,
      type,
      configuration,
    });
...
router.post(
  '/reports/:reportId/generate-data',
  asyncHandler(async (req: Request, res: Response) => {
    const { userId } = req.body;
    const { reportId } = req.params;
    if (!userId) {
    ...
    res
      .status(200)
      .json(successResponse(null, 'Rapor verileri başarıyla üretildi. (Simüle edildi)'));
  })
);

// Encryption API rotaları
router.post(
  '/encryption/master-password-setup',
  asyncHandler(async (req: Request, res: Response) => {
...
router.post(
  ...
  asyncHandler(async (req: Request, res: Response) => {
    ...
    if (!userId || !recoveryMethod || !recoveryData) {
      throw new InvalidArgumentError(
        'Kullanıcı kimliği, kurtarma metodu veya kurtarma verisi eksik.'
      );
    }
...
router.post(
  '/ai-notifications/send-recommendation',
  asyncHandler(async (req: Request, res: Response) => {
...
router.post(
  '/auto-categorization/categorize',
  asyncHandler(async (req: Request, res: Response) => {
    ...
    res
      .status(200)
      .json(successResponse(categorizedEvents, 'Etkinlikler başarıyla kategorize edildi.'));
  })
);

// Automatic Event API rotaları
router.post(
  '/automatic-events/create',
  asyncHandler(async (req: Request, res: Response) => {
    const { userId, startDate, endDate } = req.body;
    if (!userId) {
      throw new UnauthorizedError('Kullanıcı kimliği doğrulanmamış.');
    ...
    res
      ...
      .json(
        successResponse(
          createdEvents,
          'Otomatik etkinlikler başarıyla oluşturuldu. (Simüle edildi)'
        )
      );
  })
);
...
router.post(
  ...
  asyncHandler(async (req: Request, res: Response) => {
    ...
    if (!validTriggerTypes.includes(ruleData.trigger.type)) {
      throw new InvalidArgumentError(
        `Geçersiz tetikleyici türü: ${ruleData.trigger.type}. Kabul edilen türler: ${validTriggerTypes.join(', ')}`
      );
    }
...
router.post(
  '/behavioral-analysis/realtime-pattern',
  asyncHandler(async (req: Request, res: Response) => {
    const { userId, activityEvent } = req.body;
    if (!userId) {
      throw new UnauthorizedError('Kullanıcı kimliği doğrulanmamış.');
...
router.post(
  ...
  asyncHandler(async (req: Request, res: Response) => {
    ...
    res
      .status(200)
      .json(successResponse(matchedRules, 'Topluluk kuralları başarıyla eşleştirildi.'));
  })
);

// Contextual Categorization API rotaları
router.post(
  '/contextual-categorization/categorize',
  asyncHandler(async (req: Request, res: Response) => {
    const { userId, events } = req.body;
    if (!userId) {
      throw new UnauthorizedError('Kullanıcı kimliği doğrulanmamış.');
    ...
    res
      ...
      .json(
        successResponse(
          categorizedEvents,
          'Etkinlikler bağlamsal olarak başarıyla kategorize edildi.'
        )
      );
  })
);
...
router.put(
  '/custom-contextual-rules/:ruleId',
  asyncHandler(async (req: Request, res: Response) => {
    const { userId, updates } = req.body;
    const { ruleId } = req.params;
    if (!userId) {
